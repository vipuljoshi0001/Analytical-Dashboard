      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select value={form.gstPercent || 0} onChange={e => set('gstPercent', e.target.value)} className={selectClass}>
              {GST_RATES.map(r => <option key={r} value={r}>{r}%</option>)}
            </select>
          </div>
          <Input label="Cost Price (₹)" type="number" min="0" value={form.costPrice || ''} onChange={e => set('costPrice', e.target.value)} required />
          <Input label="Selling Price (₹)" type="number" min="0" value={form.sellingPrice || ''} onChange={e => set('sellingPrice', e.target.value)} required />
          <Input label="Quantity" type="number" min="0" value={form.quantity || ''} onChange={e => set('quantity', e.target.value)} required />
          <Input label="Low Stock Alert" type="number" min="0" value={form.lowStockAt || 5} onChange={e => set('lowStockAt', e.target.value)} />
        </div>
        <div className="flex gap-3 pt-2">
          <Button type="submit" loading={loading} className="flex-1 justify-center">Save Changes</Button>
